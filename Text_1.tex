...
To do this, the first phase of the task, the approach phase, is executed by a novel architecture consisting of CHECK a visual servo in a cascade with an impedance controller. The visual servo brings the robot end-effector onto the grasping point on the tumbling Target. It is widely recognized that the robustness of a visual servo is significantly improved against modelling errors when performing tracking rather than regulation~\cite{siciliano2016springer}. The use of robot impedance control is intended to minimize the detrimental effect of an impact between the robot manipulator and the Target. A second novel feature of the controller is in the use of an extended Kalman filter to robustly estimate the pose of the Target throughout the approach motion. The filter is fed with the noisy, low-frequency pose estimates of a model-based visual tracking algorithm, providing higher frequency poses to the tracking controller. The Kalman filter is designed to reduce its computational footprint, in view of an implementation on a realtime system.

In the following rigidization phase of the grasping task, in which the Target is already grasped but needs to be stabilized with respect to the Chaser, the reference trajectory brings the system into a final configuration with decaying velocity, by virtue of the robot manipulator controller's dissipation. The tracking control task comprises both these phases in sequence.

In this paper we present the tracking controller just described and its validation on DLR's OOS-SIM robotic facility, shown in Fig.~\ref{fig:facility}. This facility allows reproducing realistic orbital gravity-free dynamics in three-dimensions through hardware-in-the-loop simulation. The success of the controller requires a judicious interplay between the different elements involved: the motion planner, the visual servo (including the Kalman filter and the visual tracking) and the free-floating robot impedance controller. Furthermore, the intrinsic modelling uncertainty in the reference trajectory is handled with on-line adaptation of the reference trajectory for the rigidization phase. The OOS-SIM facility is used here with a novel momentum-based approach to simulate the post-grasping tumbling motion of the satellite stack.}

The paper is then structures as follows. In the rest of this Section, we present a relevant bibliography and a more detailed problem statement. In Section~\ref{sec:methods} we provide details of the applied methods, while in Section~\ref{sec:results} we present the experimental results and their analysis. In Section~\ref{sec:disc} we provide a discussion as well as our conclusions and views on future work. The mathematical notation is such that scalar constants are written in plain text, scalar variables in italics, vectors, vector arrays and matrices are always bold, coordinates of vector quantities are expressed with the left upper superscript. The right upper superscript $i\:j$ defined the direction of a vector from $i$ to $j$.
%
%
...
\psfrag{t_5}[cc][cc][\FontFigS]{\small{$\begin{Bmatrix}V_{e} & \dot{q} & H_{e}(H_b, q) & q\end{Bmatrix}$}}
...
\rek{In the following text, a pose matrix is denoted as, $\mathbf{H} = \begin{bmatrix}R(q) & r\\0_{1,3} & 1\end{bmatrix} \equiv \mathbf{H}(R,r)$, where $q$ and $r$ are the quaternion orientation and the position respectively. The state of the target is defined by its inertial pose, $\mathbf{H}_t = (R(q),r)$ where $q,~r$ are indicated relative to the inertial frame. The measured pose using the vision sensor is the pose of $\mathcal{G}$ relative to $\mathcal{E}$, and is denoted as $\mathbf{H}_{eg}(R(\mu),r_c)$. The forward kinematics of the chaser end-effector is $\mathbf{H}_e(R(\eta_c),\rho_c) = \mathbf{H}_{be}(q)\mathbf{H}_b$. The transformation of the grasping frame $\mathcal{G}$ relative to $\mathcal{T}$ is known from existing satellite model, and is denoted as $\mathbf{H}_{tg}(R(\eta_t),r_t)$. The pose $\mathbf{H}_t$ is driven by a mixed velocity representation $\mathbf{V}_t^m = \begin{bmatrix}\dot{r}\\ \omega^b\end{bmatrix}$, where the superfix $b$ and $m$ denote body and mixed velocity forms \rek{respectively}. The dynamic system \rek{for the EKF} is modeled with the state, \rek{$x_a \in \mathbb{R}^{13} = \begin{bmatrix} q^T& r^T& \omega^{b^T}& \dot{r}^T&\end{bmatrix}^T$} as,}

\begin{align}
\frac{d}{dt}\begin{bmatrix} \dot{q} \\ r \\ \omega^b \\ \dot{r}\end{bmatrix} = \begin{bmatrix}\frac{1}{2}\omega^b \otimes q \\ \dot{r} \\-\omega^b_\times I\omega^b \\ \Phi(n) \end{bmatrix} \label{eq_SysDyn}
\end{align}
where $\otimes$ denotes the quaternion multiplication tensor, $(.)_\times$ denotes the skew-symmetric form of $\mathbb{R}^3$. $\Phi(n)$ is the linear approximation of orbital dynamics given by the Clohessy-Wiltshire equations \rek{\cite{Aghili07}}.
\rek{\begin{assumption} \label{ass_ignore_clw}
Since the scope of this paper is limited to an operational time, $T < 20$ [sec], the contribution of the term which accounts for the relative translation motion, and is given by the Clohessy-Wiltshire equations, is neglected.
Hence, from eq.~\eqref{eq_SysDyn}
\begin{equation} \label{eq_trans_kin_ass}
\frac{d}{dt}\begin{bmatrix}r\\ \dot{r} \end{bmatrix} = \begin{bmatrix}0_{3,3} & 1_{3,3}\\ 0_{3,3}& 0_{3,3} \end{bmatrix}\begin{bmatrix}r\\ \dot{r} \end{bmatrix}
\end{equation}
\end{assumption}}

\rek{For the quaternion group, $Q$, in the multiplicative variant of the EKF, the quaternion state is maintained as a $3$-component vector as $\delta q_v(a_p) \in T_{\overline{q}}Q$, at a nominal value of the quaternion $\overline{q}$. $T_{\overline{q}}Q$ is the tangent space to the quaternion manifold and $a_p$ is a 3-component parameterization vector.} A detailed \rek{discussion} on such vector expressions was given in \cite{Markley}. In this text, the Modified Rodriguez Parameters (MRP), $a_p$ was chosen \rek{so that singularity is not encountered before angular error $2\pi$}. \rek{The tangent vector $\delta q_v$, the parameterization vector $a_p$ and the quaternion $q = \begin{bmatrix}q_v^T& q_0\end{bmatrix}$ are related as follows},
\begin{align}
